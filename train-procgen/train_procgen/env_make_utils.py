...
def get_venv_params(env):
    ...
    return {'num_levels': p['num_levels'], 'start_level': p['start_level'],
            ...
            'locacoinrun_draw_bars': p['locacoinrun_draw_bars']}, p['env_name']
...
def setup_loca(env, test_env, phase):
    ...
    env_kwargs, env_name = get_venv_params(env)
    ...
    # recreating env
    new_env = make_env(env.num_envs, env_name=env_name, **env_kwargs)

    # extract test env parameters before re-making it with new loca phase
    test_env_kwargs, env_name = get_venv_params(test_env)
    test_env_kwargs['locacoinrun_reward_phase'] = phase
    # do not restrict spawn in test envs

    # recreating test env
    new_test_env = make_env(test_env.num_envs, env_name=env_name, **test_env_kwargs)
