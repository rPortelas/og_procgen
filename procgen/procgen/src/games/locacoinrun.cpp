...
std::vector<std::string> LOCA_WALKING_ENEMIES = {"slimeBlock", "slimePurple", "slimeBlue", "slimeGreen", "mouse", "snail", "ladybug", "wormGreen", "wormPink"};
std::vector<std::string> LOCA_PLAYER_THEME_COLORS = {"Beige", "Blue", "Green", "Pink", "Yellow"};
std::vector<std::string> LOCA_GROUND_THEMES = {"Dirt", "Grass", "Planet", "Sand", "Snow", "Stone"};

const int NUM_LOCA_GROUND_THEMES = (int)(LOCA_GROUND_THEMES.size());

class LocaCoinRun : public BasicAbstractGame {
  public:
    ...
    // TODO modify to make it loca friendly
    int coin_pos = -1; // Added this to give info of which coin was collected (0==left, 1==right)
    float init_agent_x = -1;
    int initially_closest_coin = -1; // Added this to know whether the agent learns to go to the closest coin
    ...
    void asset_for_type(int type, std::vector<std::string> &names) override {
        if (type == PLAYER) {
            for (const auto &color : LOCA_PLAYER_THEME_COLORS) {
            ...
            for (const auto &enemy : LOCA_WALKING_ENEMIES) {
            ...
            names.push_back("kenney/Items/coinBronze.png");
        } else if (type == WALL_TOP) {
            for (const auto &ground : LOCA_GROUND_THEMES) {
...
//                std::cout << "agent init pos is " << init_agent_x << "\n";
//                std::cout << "left coin pos is " << leftcoin_pos << "\n";
//                std::cout << "right coin pos is " << rightcoin_pos << "\n";

                if (agent->x < 3){
                    //std::cout << "we got left coin";
                    coin_pos = 0; // agent collected leftmost coin
                    //std::cout << coin_pos;
                } else{
                    //std::cout << "we got left coin";
                    coin_pos = 1; // agent collected rightmost coin
                    //std::cout << coin_pos;
                }
                if ((rightcoin_pos - init_agent_x) > (init_agent_x - leftcoin_pos)){
                    initially_closest_coin = 0;
                } else {
                    initially_closest_coin = 1;
                }
    ...
    void game_draw(QPainter &p, const QRect &rect) override {
        ...
        if (options.locacoinrun_draw_bars){
            //std::cout<< "OPTION" << options.locacoinrun_draw_bars << "\n";
            QColor right_color = QColor(255, 215, 0);
            QColor left_color = QColor(205, 127, 50);

            float left_bar_height = ((agent->x - leftcoin_pos) / 3.5) + 0.5;
            float right_bar_height = ((rightcoin_pos - agent->x) / 3.5) + 0.5;

            QRectF dist_rect2 = get_abs_rect(.25, visibility - .5 - left_bar_height, .5, left_bar_height);
            QRectF dist_rect3 = get_abs_rect(12.25, visibility - .5 - right_bar_height, .5, right_bar_height);
            p.fillRect(dist_rect2, left_color);
            p.fillRect(dist_rect3, right_color);
        }
    }
    ...
    void generate_coin_to_the_right() {
        ...
        int num_sections = rand_gen.randn(dif) + dif + 2; // add +2 to avoid trivial 1 & 2 section envs with coins directly on agent
        int curr_x = 1; // changed this to 1 to erase the start pad during section generation
        int curr_y = 10; // changed this to 10 to allow so that leftmost sections can have pits
        ...
        int random_agent_position = rand_gen.randn(num_sections);
        ...
        for (int section_idx = 0; section_idx < num_sections; section_idx++) {
            ...
            if (section_idx == 0){
                set_obj(curr_x, curr_y, GOAL2); //add another coin to the leftmost side
                leftcoin_pos = curr_x;
            }
            ...
            if (use_pit) {
                ...
                if (rand_gen.randn(10) < (2 * dif) && dx > 3) {
                    ...
                    //add some logic to prevent spawning near a saw if at the last section
                    if ((section_idx == (num_sections - 1)) || ((curr_x + dx + 15) >= w)){
                        if ((options.locacoinrun_restrict_spawn) || (random_agent_position == section_idx)){
                            ob1_x += 1;
                        }
                    }
            ...
            if (section_idx == random_agent_position){
                if (agent_placed == false && options.locacoinrun_restrict_spawn == false){
                    // position agent either on the leftmost or rightmost section side, while avoiding to be at the coin pos
                    int agent_x = -1;
                    if (section_idx == 0){
                        agent_x = curr_x + dx - 1;
                    } else if ((section_idx == (num_sections - 1)) || ((curr_x + dx + 15) >= w)){
                        agent_x = curr_x;
                    } else {
                        if (rand_gen.randn(2) == 1){
                            agent_x = curr_x;
                        } else {
                            agent_x = curr_x + dx - 1;
                        }
                    }
                    agent->x = agent_x + agent->rx;
                    agent->y = curr_y + agent->ry;
                    agent_placed = true;
                }
            }
            if ((section_idx == (num_sections - 1)) || ((curr_x + dx + 15) >= w)){
                last_section_x = curr_x;
                if (options.locacoinrun_restrict_spawn){
                    fassert(agent_placed == false);
                    agent->x = last_section_x + agent->rx;
                    agent->y = curr_y + agent->ry;
                    agent_placed = true;
                }
            }
        ...
        rightcoin_pos = curr_x;

        init_agent_x = agent->x;
    ...
    void game_reset() override {
        ...
        if (options.distribution_mode == EasyMode) {
            ...
            wall_theme = rand_gen.randn(NUM_LOCA_GROUND_THEMES);
        }
    ...
    void game_step() override {
        ...
        // one-way passage at the beginning of the last section
        bool zero_vx = false;
        if ((agent->x > last_section_x) && ((agent->x - last_section_x) < 0.5)){
            if (agent->vx < 0){
                //std::cout << "vx:" << agent->vx << "x:" << agent->x << "last_x:" << last_section_x << "\n";
                agent->vx = 0;
                zero_vx = true;
                agent->x += 0.25;
                //std::cout << "new vx:" << agent->vx << "\n";
            }
        }
        ...
        //std::cout << "post_step vx:" << agent->vx << "\n";
    ...
    Game::observe();
    //std::cout << "hello";
    //std::cout << step_data.done;
    //std::cout << coin_pos;
    if (step_data.done) {
            *(int32_t *)(info_bufs[info_name_to_offset.at("locacoinrun_coin_pos")]) = coin_pos;
            *(int32_t *)(info_bufs[info_name_to_offset.at("locacoinrun_initially_closest_coin")]) = initially_closest_coin;
            coin_pos = -1; // reset collected coin pos info
            initially_closest_coin = -2;
    }
...
REGISTER_GAME(NAME, LocaCoinRun);
