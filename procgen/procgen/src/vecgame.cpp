...
VecGame::VecGame(int _nenvs, VecOptions opts) {
    ...
    {
    // Adding info type to store whether it was the left or right coin that was collected
    struct libenv_tensortype s;
    strcpy(s.name, "locacoinrun_coin_pos");
    s.scalar_type = LIBENV_SCALAR_TYPE_DISCRETE;
    s.dtype = LIBENV_DTYPE_INT32;
    s.ndim = 0,
    s.low.int32 = 0;
    ...
    // Adding info type to store whether it was the left or right coin that was collected
    struct libenv_tensortype s;
    strcpy(s.name, "locacoinrun_initially_closest_coin");
    s.scalar_type = LIBENV_SCALAR_TYPE_DISCRETE;
    s.dtype = LIBENV_DTYPE_INT32;
    s.ndim = 0,
    s.low.int32 = 0;
